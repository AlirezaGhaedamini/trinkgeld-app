-- ═════════════════════════════════════════════════════════════════════════════
-- 16 · pairwise overlap, and no silent redistribution
--
-- Three product decisions, implemented. Migration 15's integrity fixes are
-- preserved untouched; this only replaces calculate_distribution() and widens
-- app.engine_supports_basis().
--
-- ── 1 · PAIRWISE OVERLAP ────────────────────────────────────────────────────
--
-- Selectable, not imposed: `longest_shift` remains the column default and the
-- model every existing rule uses. A manager switches a workplace to `pairwise`
-- by activating a rule that names it, which is an explicit, versioned act.
--
-- THE ALGORITHM, EXACTLY
--
--   1. Candidates are the same as before: approved shifts in the pool period
--      with worked_minutes > 0, resolved to an effective area
--      (shift.area_id ?? member.area_id) and an effective role.
--
--   2. For every unordered pair of DISTINCT members (P, Q):
--
--        overlap(P, Q) = Σ  |shift_p ∩ shift_q|   in whole minutes
--
--      summed over every combination of P's shifts and Q's shifts. A person's
--      own shifts cannot overlap each other — the exclusion constraint in
--      migration 08 forbids it — so this sum is exactly the measure of the
--      time the two people were both at work. Multiple shifts per person,
--      several partial overlaps and shifts in different areas all fall out of
--      this correctly; an overnight shift is one interval like any other,
--      because the intervals are instants and not clock faces.
--
--   3. P and Q are LINKED when overlap(P, Q) >= rule.min_overlap_minutes.
--      Exactly at the threshold is linked; one minute below is not.
--
--   4. Eligibility.
--        · If the period has exactly ONE candidate member, they are eligible.
--          There is nobody to overlap with, and a person who worked alone has
--          not thereby forfeited the tips.
--        · Otherwise a member is eligible when they have at least one link.
--          A member with no link at all is excluded, and the snapshot records
--          the reason as `no_pairwise_overlap`.
--
--   5. Connectivity. The eligible members form a graph. Chains are intended:
--      if A—B and B—C are links, A, B and C are all in the same group even
--      though A and C never met — B is what makes it one shift's worth of work.
--
--      If the eligible members fall into TWO OR MORE disconnected groups, the
--      distribution is REFUSED. Two crews who never met cannot both be the
--      crew that earned one pool, and picking one silently is exactly the kind
--      of invisible decision this product exists to remove. The manager is
--      told to split the period into separate pools.
--
--   6. Weighting is unchanged and stays worked time × role points × personal
--      multiplier. The anchor never entered the weighting; what changed is
--      only which people are in. Each entry records that member's LARGEST
--      pairwise overlap in `overlap_minutes`, so the row still carries the
--      number that justified their inclusion.
--
--   The whole overlap graph goes into `inputs_snapshot.pairs`, so a
--   distribution can be re-argued years later without recomputing anything.
--
-- ── 2 · AN AREA WITH A SHARE BUT NOBODY IN IT ───────────────────────────────
--
-- Migration 11 absorbed such an area's percentage proportionally into the
-- areas that did have people. That is money moving without anyone deciding it
-- should. It now raises, naming the area, for BOTH overlap models.
--
-- ── 3 · THE SHIFT-LEVEL AREA OVERRIDE ───────────────────────────────────────
--
-- Unchanged and re-asserted by the tests in 06_pairwise_overlap.sql: an
-- employee may still state which area they worked; only an approved shift
-- reaches a distribution; the entry snapshots the effective area and whether it
-- came from the shift or the member; and an approved or locked shift is no
-- longer theirs to edit.
-- ═════════════════════════════════════════════════════════════════════════════
...
immutable
set search_path = ''
as $$ select p_basis in ('longest_shift', 'pairwise') $$;

comment on function app.engine_supports_basis(public.overlap_basis) is
  'Which overlap models calculate_distribution actually implements. '
  'service_window is still only an enum value.';
